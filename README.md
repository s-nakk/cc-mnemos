...
- Source-aware memory records: new sessions store whether they came from Claude or Codex, and existing sessions are backfilled with confidence-scored source classification
...
- Search results include source metadata such as `effective_source`, `recorded_source`, and inferred classification confidence
...
- 記録元の識別: 新規セッションは Claude / Codex を確定保存し、既存セッションは信頼度付きで後付け分類
...
- 検索結果には `effective_source`、`recorded_source`、推定信頼度などの source メタデータが含まれる
